...
// ** Actions Imports
import { DeriveDoc, ObservedDoc, SendDoc, addDoc } from 'src/store/apps/doc'
...
const fileObserved = (props: SidebarAddUserType) => {
  ...
  let a: string = ''
  let pasoActual: number = 0
  //console.log(pasos)
  const bitacora = doc?.bitacoraWorkflow
  if (bitacora) {
    let NroPasos = bitacora.length

    for (let i = bitacora.length; i <= bitacora.length; i++) {
      const paso = bitacora[NroPasos - 1]
        ...
        if (oficinaActual !== ultimaOficina) {
          for (let j = 0; j < oficinas.length - 1; j++) {
            if (oficinaActual === oficinas[j].idOffice) {
              const paso2 = parseInt(oficinas[j].paso, 10)
              pasoActual = paso2 - 1
  ...
  const handleSubmit = () => {
    ...
    dispatch(ObservedDoc({ docId, paso, data }))
  ...
  const [motivoObs, setMotivoObs] = useState<obsFile>({
    motivo: ''
  })
