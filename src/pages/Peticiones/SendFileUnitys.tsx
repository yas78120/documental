...
// ** Actions Imports
import { SendDoc, SendDocUnitys, SendDocUser, addDoc } from 'src/store/apps/doc'
...
const SidebarAddUser = (props: SidebarAddUserType) => {
  ...
  const handleUnitySelect = (event: SelectChangeEvent<string[]>, child: React.ReactNode) => {
    const newValue = event.target.value as string[]
    setSelectedUnity(newValue)
    console.log(newValue)
    setOpenSelect(false)
  }

  const handleSubmit = () => {
    const data: docUnitys = {
      unitys: selectedUnity
    }
    toggle()
    console.log(data)

    dispatch(SendDocUnitys({ docId, data }))
  ...
  return (
    <Dialog
      ...
      sx={{ '& .MuiPaper-root': { width: '100%', maxWidth: 600, p: [2, 4], height: '50%', maxHeight: 400 } }}
      ...
      <Box sx={{ p: 5 }}>
        <form>
          <FormControl fullWidth sx={{ mb: 6 }}>
            <InputLabel>Seleccionar Unidades de destino</InputLabel>
            <Select
              multiple
              value={selectedUnity}
              onChange={handleUnitySelect}
            ...
            >
              {groupUnity.map(unity => (
                <MenuItem key={unity._id} value={unity.name}>
                  {unity.name}
                </MenuItem>
              ))}
            </Select>
